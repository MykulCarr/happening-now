(() => {
  ...
  function destroyCriticalTicker(){
    ...
    if(criticalTickerState.resumeTimer){
      window.clearTimeout(criticalTickerState.resumeTimer);
    }
    if(criticalTickerState.rafId){
      window.cancelAnimationFrame(criticalTickerState.rafId);
    }
    if(criticalTickerState.abortController){
      criticalTickerState.abortController.abort();
    }
  ...
  function setupCriticalTicker(container){
    ...
    function queueAutoResume(delayMs = 700){
      clearResumeTimer();
    ...
    function tick(timestamp){
      ...
      if(criticalTickerState){
        criticalTickerState.rafId = rafId;
        criticalTickerState.resumeTimer = resumeTimer;
      }
    }
    ...
    viewport.addEventListener("pointerdown", (event) => {
      ...
      dragDistancePx = 0;
      pointerDownUrl = event.target?.closest(".criticalTickerItem")?.getAttribute("href") || "";
