...
- Confirm site loads:
  ...
  (the .html forms now 301 to these; a 301 in a check is expected, not a fault)
...
- Read the curation digest email (sent automatically to the address in
  security.txt; see `cloudflare-sync-worker/src/curate.js`). It lists feeds that
  stopped returning items, replacement feeds the outlet now advertises, and new
  Patch coverage. Nothing in it is applied automatically — it's for you to
  approve or ignore.
